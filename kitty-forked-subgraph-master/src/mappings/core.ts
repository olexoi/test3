...
import {
  ...
  Mint as MintEvent,
  Burn as BurnEvent,
  Exchange as SwapEvent,
  Bundle
} from '../types/schema'
import { Pair as PairContract, Mint, Burn, Exchange, Transfer, Sync } from '../types/templates/Pair/Pair'
...
export function handleSwap(event: Exchange): void {
  let pair = Pair.load(event.address.toHexString())
  let token0 = Token.load(pair.token0)
  let token1 = Token.load(pair.token1)
