...
interface IMigratorChef {
    // Perform LP token migration from legacy KittyExchange to MeowSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to KittyExchange LP tokens.
    // MeowSwap must mint EXACTLY the same amount of MeowSwap LP tokens or
...
contract MasterChef is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 meowReward = multiplier.mul(meowPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        meow.mint(devaddr, meowReward.div(10));
        meow.mint(address(mrr), meowReward);
    ...
    function safeMeowTransfer(address _to, uint256 _amount) internal {
        mrr.safeMeowTransfer(_to, _amount);
    }
