...
contract('MasterChef', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        ...
        this.chef = await MasterChef.new(this.meow.address, this.mrr.address, dev, '1000', '300', { from: minter });
        await this.meow.transferOwnership(this.chef.address, { from: minter });
        await this.mrr.transferOwnership(this.chef.address, { from: minter });
    ...
    it('real case', async () => {
      ...
      assert.equal((await this.meow.balanceOf(alice)).toString(), '0');
      await this.chef.deposit(1, '20', { from: alice });
      await this.chef.withdraw(1, '20', { from: alice });
      assert.equal((await this.meow.balanceOf(alice)).toString(), '263');

      await this.meow.approve(this.chef.address, '1000', { from: alice });
      await this.chef.enterStaking('20', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      assert.equal((await this.meow.balanceOf(alice)).toString(), '993');
    ...
    it('deposit/withdraw', async () => {
      ...
      assert.equal((await this.meow.balanceOf(alice)).toString(), '999');
      assert.equal((await this.meow.balanceOf(dev)).toString(), '100');
      ...
      assert.equal((await this.meow.balanceOf(bob)).toString(), '125');
    ...
    it('staking/unstaking', async () => {
      ...
      await this.meow.approve(this.chef.address, '250', { from: alice });
      await this.chef.enterStaking('240', { from: alice }); //3
      assert.equal((await this.mrr.balanceOf(alice)).toString(), '240');
      assert.equal((await this.meow.balanceOf(alice)).toString(), '10');
      await this.chef.enterStaking('10', { from: alice }); //4
      assert.equal((await this.mrr.balanceOf(alice)).toString(), '250');
      assert.equal((await this.meow.balanceOf(alice)).toString(), '249');
      await this.chef.leaveStaking(250);
      assert.equal((await this.mrr.balanceOf(alice)).toString(), '0');
      assert.equal((await this.meow.balanceOf(alice)).toString(), '749');
    ...
    it('updaate multiplier', async () => {
      ...
      await this.meow.approve(this.chef.address, '100', { from: alice });
      await this.meow.approve(this.chef.address, '100', { from: bob });
      ...
      assert.equal((await this.meow.balanceOf(alice)).toString(), '700');
      assert.equal((await this.meow.balanceOf(bob)).toString(), '150');
